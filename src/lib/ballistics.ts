...
/** Minimum ballistic coefficient to prevent division by zero or unrealistic values */
export const BC_MIN = 0.01
/** Minimum velocity (fps) to prevent numerical instability in drag calculations */
export const MIN_VELOCITY_FPS = 1.0
/** Default speed of sound (fps) at sea level, 59°F */
export const DEFAULT_SPEED_OF_SOUND_FPS = 1116.4
...
/**
 * Performs linear interpolation on drag coefficient table for a given Mach number.
 * 
 * @param dragTable - Array of [Mach, drag coefficient] pairs
 * @param mach - Current Mach number
 * @returns Interpolated drag coefficient
 */
...
/**
 * Gets the drag coefficient for a given velocity and ballistic coefficient type.
 * 
 * @param velocityFps - Projectile velocity in feet per second
 * @param bcType - Ballistic coefficient type ('G1' or 'G7')
 * @param speedOfSound - Speed of sound in feet per second
 * @returns Drag coefficient from the appropriate drag table
 */
export function getDragCoefficient(velocityFps: number, bcType: 'G1' | 'G7', speedOfSound: number): number {
  // Guard against division by zero
  const safeSpeedOfSound = Math.max(speedOfSound, MIN_VELOCITY_FPS)
  const mach = velocityFps / safeSpeedOfSound
...
function normalizeInputs(data: BallisticData) {
  ...
  // Guard BC to prevent division by zero or unrealistic values
  const ballisticCoefficient: number = Math.max(data.ballisticCoefficient ?? 0.475, BC_MIN)
 ...
 * 
 * Uses the formula: speed_of_sound = DEFAULT_SPEED_OF_SOUND_FPS * sqrt(T/518.67)
 * where T is temperature in Rankine (Fahrenheit + 459.67)
 * 
 * @param tempF - Temperature in degrees Fahrenheit
 * @returns Speed of sound in feet per second
 */
export function calculateSpeedOfSound(tempF: number): number {
...
function stepDrag(
  ...
  for (let i = 0; i < subSteps; i++) {
    // Guard velocity to prevent numerical instability
...
function solveBoreAngle(
  ...
  // Guard BC to prevent division by zero
  const safeBC = Math.max(bc, BC_MIN)
  ...
  for (let iter = 0; iter < 24; iter++) {
    ...
    while (st.x_ft < xZero_ft) {
      ...
      st = stepDrag(st, step, safeBC, bcType, densityRatio, speedOfSound)
